...
const ServiceCard = ({ title, desc, icon, classes, link = false, url }) => {
    ...
    if (link) {
      return (
        <a
          href={url}
          target="_blank"
          className={`${serviceStyles.serviceLink} flex flex-col items-center justify-center w-full md:flex-row ${classes} md:my-12`}
        >
          ...
          <div className="text-center text-base max-w-[300px] md:max-w-[380px] md:basis-1/2 mx-4">
  ...
  return (
    <article
      className={`my-6 py-6 px-4 border-black-800 border-solid border-2 flex items-center justify-center flex-col w-full first:mt-0 md:border-transparent ${classes} ${serviceStyles.articleLink} md:w-[100%] md:w-full md:flex-row md:max-w-[100%] `}
    >
      {generateCard(link)}
    </article>
  );
